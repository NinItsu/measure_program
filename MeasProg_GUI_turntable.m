function varargout = MeasProg_GUI_turntable(varargin)
% MEASPROG_GUI_TURNTABLE MATLAB code for MeasProg_GUI_turntable.fig
%      MEASPROG_GUI_TURNTABLE, by itself, creates a new MEASPROG_GUI_TURNTABLE or raises the existing
%      singleton*.
%
%      H = MEASPROG_GUI_TURNTABLE returns the handle to a new MEASPROG_GUI_TURNTABLE or the handle to
%      the existing singleton*.
%
%      MEASPROG_GUI_TURNTABLE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MEASPROG_GUI_TURNTABLE.M with the given input arguments.
%
%      MEASPROG_GUI_TURNTABLE('Property','Value',...) creates a new MEASPROG_GUI_TURNTABLE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before MeasProg_GUI_turntable_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to MeasProg_GUI_turntable_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help MeasProg_GUI_turntable

% Last Modified by GUIDE v2.5 27-Dec-2021 19:16:09
global version;
version = { 'Measure Program for Turntable';...
            'v 1.0';...
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @MeasProg_GUI_turntable_OpeningFcn, ...
                   'gui_OutputFcn',  @MeasProg_GUI_turntable_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before MeasProg_GUI_turntable is made visible.
function MeasProg_GUI_turntable_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to MeasProg_GUI_turntable (see VARARGIN)

% Choose default command line output for MeasProg_GUI_turntable
...
% UIWAIT makes MeasProg_GUI_turntable wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = MeasProg_GUI_turntable_OutputFcn(hObject, eventdata, handles) 
...
function Rotate_Interval_Callback(hObject, eventdata, handles)
    
% hObject    handle to Rotate_Interval (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Rotate_Interval as text
%        str2double(get(hObject,'String')) returns contents of Rotate_Interval as a double
    RI_STR=get(hObject,'String');
    set(hObject,'UserData',RI_STR);

% --- Executes during object creation, after setting all properties.
function Rotate_Interval_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Rotate_Interval (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function Rotate_Min_Callback(hObject, eventdata, handles)
   
% hObject    handle to Rotate_Min (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Rotate_Min as text
%        str2double(get(hObject,'String')) returns contents of Rotate_Min as a double
    RMN_STR=get(hObject,'String');
    set(hObject,'UserData',RMN_STR);

% --- Executes during object creation, after setting all properties.
function Rotate_Min_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Rotate_Min (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function Rotate_Max_Callback(hObject, eventdata, handles)
% hObject    handle to Rotate_Max (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Rotate_Max as text
%        str2double(get(hObject,'String')) returns contents of Rotate_Max as a double
     RMX_STR=get(hObject,'String');
     set(hObject,'UserData',RMX_STR);


% --- Executes during object creation, after setting all properties.
function Rotate_Max_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Rotate_Max (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
%%%%%最小角度
RMN_STR=get(handles.Rotate_Min,'String');
rot.MIN=str2double(RMN_STR);
params.rot.MIN=rot.MIN;

%%%%%最大角度
RMX_STR=get(handles.Rotate_Max,'String');
rot.MAX=str2double(RMX_STR);
params.rot.MAX=rot.MAX;

%%%%%測定の角度間隔
RI_STR=get(handles.Rotate_Interval,'String');
rot.INR=str2double(RI_STR);
params.rot.INR=rot.INR;

if isnan(rot.MIN) %%数字じゃないとエラー
      error=1;
      set(handles.Rotate_Min,'ForegroundColor','red');
else
      set(handles.Rotate_Min,'ForegroundColor','black');
end

if isnan(rot.MAX) %%数字じゃないとエラー
      error=1;
      set(handles.Rotate_Max,'ForegroundColor','red');
else
      set(handles.Rotate_Max,'ForegroundColor','black');
end

%%%%%最大角と最小角の差が回転角で割り切れない時エラー
if mod((rot.MAX-rot.MIN),rot.INR) ~=0 || xor(rot.INR<0,rot.MIN>rot.MAX)
    error=1;
      set(handles.Rotate_Interval,'ForegroundColor','red');
else
      set(handles.Rotate_Interval,'ForegroundColor','black');
end
...
set(handles.Rotate_Interval,'String','');
set(handles.Rotate_Min,'String','0');
set(handles.Rotate_Max,'String','360');
global set_LSP;
set_LSP=[];
global set_MIC;
set_MIC=[];
...
set(handles.Rotate_Interval,'String',num2str(params.rot.INR));
set(handles.Rotate_Min,'String',num2str(params.rot.MIN));
set(handles.Rotate_Max,'String',num2str(params.rot.MAX));
global set_LSP;
